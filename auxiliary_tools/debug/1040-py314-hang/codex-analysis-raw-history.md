...
You are working in the e3sm_diags repo. I want you to focus on two suspected root causes of Python 3.14 instability on HPC:

(1) GC-dependent dataset cleanup: xarray/open_dataset/open_mfdataset objects not deterministically closed, leading to FD leaks, cached backend handles, or lock lifetimes that depend on GC/finalization timing.
(2) open_mfdataset() in loops + eager load (.load(scheduler="sync")) without explicit close, especially across many variables/years/regions.

Goal: identify exact code paths and propose a minimal, safe patch that ensures deterministic close/cleanup without changing output semantics.

Do this:

A) Locate and summarize all dataset-open sites in core runtime:
- xr.open_dataset
- xr.open_mfdataset
- netCDF4.Dataset
- any helper wrappers around these
For each site, report:
- file + line range
- whether a context manager is used
- whether .close() is called explicitly
- whether the object escapes scope (returned/stored globally/cached)
- whether it’s inside loops over variables/years/regions/sets

B) Identify high-risk patterns:
- open_mfdataset inside loops
- .load() called but dataset not closed
- datasets cached in long-lived objects (class attributes, module-level singletons, LRU caches)
- delayed cleanup via __del__ or reliance on GC
- xarray backend usage that may keep file handles open (e.g., netCDF4 engine default)

C) Produce a patch plan (minimal changes):
- Add explicit close in finally blocks, or convert to `with xr.open_dataset(...) as ds:` patterns.
- If code needs loaded arrays after closing, use a safe approach:
  - `ds.load()` then either `ds = ds.copy(deep=True)` or extract needed DataArrays and return those, then close original.
  - Ensure `.close()` is called even on exceptions.
- Prefer local helper functions like `open_load_close(...)` to reduce duplication.

D) Implement the patch (or a draft PR diff) for the highest-impact locations first:
- dataset_xr.py open sites (notably the open_mfdataset + load paths)
- any netCDF4.Dataset open without context manager (e.g., tc_analysis_driver)
Keep changes minimal and add comments explaining why explicit close is needed.

E) Add optional lightweight instrumentation (guarded by env var, e.g., E3SM_DIAGS_DEBUG_RESOURCES=1):
- Log open file descriptor count on Linux: len(os.listdir("/proc/self/fd"))
- Log RSS via psutil or resource.getrusage
- Log before/after each dataset open/load/close boundary
Do not enable by default; only when env var is set.

Output:
1) A table of all open sites with “needs fix” yes/no.
2) A short rationale tying fixes to “Slurm cancelled / BrokenProcessPool” symptoms (FD exhaustion, OOM, lock contention).
3) A unified diff (git-style) for the proposed minimal patch set.