...
# bash auxiliary_tools/debug/1048-py314-stall-cont/run_xarray_release_qa_lcrc_py3143.sh --repro-runs 3 --timeout 60m

set -euo pipefail

readonly DEFAULT_SRUN_TIME="01:00:00"
readonly DEFAULT_TIMEOUT="30m"
readonly DEFAULT_REPRO_RUNS="1"
...
usage() {
  cat <<'EOF'
Usage: run_xarray_release_qa_lcrc_py3143.sh [options]

Run the Python 3.14.3 conda xarray release matrix for the #1048 Chrysalis/LCRC
minimum repro using `srun --pty --nodes=1 --time=01:00:00 /bin/bash`.
...
Options:
  ...
  --repro-runs N          Number of qa.py iterations per env (default: 1).
                         For intermittent stalls, try `--repro-runs 5`
                         with a larger timeout such as `--timeout 60m`.
...
log() {
  printf '[run_xarray_release_qa_lcrc_py3143] %s\n' "$*"
}


die() {
  printf '[run_xarray_release_qa_lcrc_py3143] Error: %s\n' "$*" >&2
  exit 1
}
...
init_paths() {
  ...
  RUN_DIR="${RUNS_ROOT}/xarray-release-py3143-lcrc-${timestamp}"
...
xarray_descriptor_for_env() {
  ...
  case "${env_name}" in
    ...
    ed_1048_xr_latest_2026070_py3143)
      ...
      ;;
    *)
      die "Unknown environment for xarray descriptor mapping: ${env_name}"
      ;;
  esac
}


xarray_subject_for_env() {
  local env_name=$1

  case "${env_name}" in
    ed_1048_xr_2025120_py3143)
      printf '%s\n' "Conda package from dev_latest.yml with python=3.14.3 and xarray=2025.12.0"
      ;;
    ed_1048_xr_2026010_py3143)
      printf '%s\n' "Conda package from dev_latest.yml with python=3.14.3 and xarray=2026.01.0"
      ;;
    ed_1048_xr_latest_2026070_py3143)
      printf '%s\n' "Conda package from dev_latest.yml with python=3.14.3 and xarray=2026.07.0"
      ;;
    *)
      die "Unknown environment for xarray subject mapping: ${env_name}"
      ;;
  esac
}
...
render_command_script() {
  ...
  local run_log=$3
  local status_file=$4
  local xarray_descriptor
  local xarray_subject

  xarray_descriptor=$(xarray_descriptor_for_env "${env_name}")
  xarray_subject=$(xarray_subject_for_env "${env_name}")
...
readonly XARRAY_DESCRIPTOR="${xarray_descriptor}"
readonly XARRAY_SUBJECT="${xarray_subject}"
...
echo "Expected xarray descriptor: \${XARRAY_DESCRIPTOR}"
echo "Expected xarray subject: \${XARRAY_SUBJECT}"
