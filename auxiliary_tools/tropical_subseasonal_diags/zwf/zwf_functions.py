...
#    i_frq03 = getNearestInd1D(data['frequency'], 0.3)   # index of 'frequency' coord closest to 0.3
    i_minwav4smth = int(np.where(data['wavenumber']==-27)[0])  # index of 'wavenumber' coord where it equals -27
    i_maxwav4smth = int(np.where(data['wavenumber']==27)[0])   # index of 'wavenumber' coord where it equals 27
    ...
    for i in range(i_frq0+1,nfrq):       # Loop over all positive frequencies
      ...
      if(data['frequency'][i] < 0.1):
        nsmth_iter = 5
        print("  Wavenumber smoothing " + str(nsmth_iter) + " times for freq: " + str(float(data['frequency'][i])))
        for smth_iter in range(nsmth_iter):
          for j in range(i_minwav4smth, i_maxwav4smth+1):
            data[j,i] = 0.25 * data[j-1,i] + 0.5 * data[j,i] + 0.25 * data[j+1,i]
    ...
    nsmth_iter = 10
    print("Frequency smoothing " + str(nsmth_iter) + " times for all wavenumbers (pos frqs only)")
    for smth_iter in range(10):
      data[:,i_frq0+1] = 0.75 * data[:,i_frq0+1] + 0.25 * data[:,i_frq0+2]
      data[:,-1]       = 0.25 * data[:,-2] + 0.75 * data[:,-1]
      for i in range(i_frq0+2, nfrq-1):
...
def resolveWavesHayashi( varfft: xr.DataArray, nDayWin: int, spd: int ) -> xr.DataArray:  
    ...
    varspacetime[..., 0:mlon//2, N//2:     ] = varfft[..., mlon//2:0:-1, 0:N//2+1]   # pos.k,
    ...
    pee       = (np.abs(varspacetime))**2   # JJB: abs(a+bi) = sqrt(a**2 + b**2), which is what is done in NCL's resolveWavesHayashi
...
def spacetime_power(data, segsize=96, noverlap=60, spd=1, latitude_bounds=None, dosymmetries=False, rmvLowFrq=False):
    ...
    stride_steps   = segsize_steps - noverlap_steps  # Stride for overlapping windows, in time step units
    ...
    x_roll = data.rolling(time=segsize_steps, min_periods=segsize_steps)  # WK99 use 96-day window
    x_win = x_roll.construct("segments", stride=stride_steps).dropna("time")  # WK99 say "2-month" overlap; dims: (nSegments,nlat,nlon,segment_length_steps)
    ...
    if  np.logical_not(np.any(np.isnan(x_win))):
        ...
        x_win_detr = detrend(x_win.values, axis=-1, type='linear') #<-- missing data makes this not work; axis=-1 for segment window dimension
    ...
    taper = split_hann_taper(segsize_steps, 0.1)  # try to replicate NCL's approach of tapering 10% of segment window
    ...
    z = np.fft.fft(x_wintap, axis=2) / lon_size  # note that np.fft.fft() produces same answers as NCL cfftf;  axis=2 to do fft along dim=lon
    z = np.fft.fft(z, axis=3) / segsize_steps    # axis=3 to do fft along dim=segment_length_steps
