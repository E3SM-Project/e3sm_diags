...
model_hostpath='/global/cfs/cdirs/e3sm/user/location_ofregriddedh3data'   #'/directory/model_data/'
...
if PrecipPDF_YorN>0:
    ...
    #   **********************************************************************
    fis=glob.glob(os.path.join(model_hostpath,'*20*')) #find gridded data - USERS - if daily mean is not in h1 file, modify
    ...
    for fi in fis:  #Create a netcdf with the pdfs for each model netcdf file
        ...
        before_str,after_str=fi.split('.eam.h3.')
