...
#        ncap2 -O -s 'where(lon > 180) lon=lon-360' ${tmp}GPCP_v3.2_${yyyy}${mm}.nc ${tmp}GPCP_v3.2_${yyyy}${mm}.nc
#    done
#done
...
#for j in "${sn[@]}"
#do
#    ncks -O --msa_usr_rdr -d lon,0.25,179.75 -d lon,-179.75,-0.25 ${climo_output_path}GPCP_v3.2_${j}_*nc ${tmp}GPCP_v3.2_${j}_*nc
#    ncap2 -O -s 'where(lon > 180) lon=lon-360' ${tmp}GPCP_v3.2_${j}_*nc ${tmp}out_${j}.nc
#    
#done
exit
