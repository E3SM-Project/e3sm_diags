...
#new_grids = cdms2.createUniformGrid(90.0, 23, -8.0, 0.0, 36, 10.0,order='yx')
new_grids = cdms2.createUniformGrid(90.0, 36, -5.0, 0.0, 60, 6.0,order='yx')
x1_new = x1.regrid(new_grids,regridMethod='bilinear', regridTool = 'esmf')
y1_new = y1.regrid(new_grids,regridMethod='bilinear', regridTool = 'esmf')
#y1 = fin1('TAUY')
#
#x2 = fin2('tauu')
#y2 = fin2('tauv')
...
#var = add_cyclic(var)
lon = x1_new.getLongitude()
lat = x1_new.getLatitude()
#convert surface stress to wind stress
u = -ma.squeeze(x1_new.asma())
v = -ma.squeeze(y1_new.asma())
wind = (u**2+v**2)**0.5
u_norm = u/wind
v_norm = v/wind
...
ax.quiver(x[skip],y[skip],u[skip],v[skip],transform=ccrs.PlateCarree())#,units = 'xy' )#, scale_units = 'inches', scale = 0.5, pivot = 'middle')#, scale = 50)#,color='black', headwidth=1, scale = 100, headlength=2)
ax.set_aspect('auto')
...
#ax.quiver(x,y,u,v)
ax1.quiver(x[skip],y[skip],u_norm[skip],v_norm[skip], transform=ccrs.PlateCarree(),units ='xy')#,width = 0.002, scale = 50)#,color='black', headwidth=1, scale = 100, headlength=2)
ax1.set_aspect('auto')
