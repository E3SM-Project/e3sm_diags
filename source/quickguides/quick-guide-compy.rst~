...
Then, following the instructions on screen if you are setting up conda for the first time:
...
Most of the E3SM analysis software is maintained with an Anaconda metapackage. If you have an account on an E3SM supported machines(**Cori, Compy, Acme1, Anvil, Cooley, Rhea**), to get all of the tools in the metapackage in your path, use the command below and the activation path from below for Compy. 
...
Additionally, following are relevent paths and web address for running diags and visualize diags results.
...
Once you ran the diagnostics in an interactive session or via a batch job, open the following webpage served at : https://compy-dtn.pnl.gov/<username> to view the results.
