...
Adjust any options as you like.
   ...
   **Tip:** Some of E3SM's analysis machines (**Acme1, Anvil, Compy, Cori**)
   ...
   On Compy, create the directory ``#expand html_path`` using your username.
   Set ``results_dir`` to ``/compyfs/www/<username>/lat_lon_demo``
   in ``run_e3sm_diags.py`` below. Then, you can view results via a web browser here:
   https://compy-dtn.pnl.gov/<username>/lat_lon_demo
...
open  ``https://compy-dtn.pnl.gov/<username>/lat_lon_demo/viewer/index.html`` to view the results.
...
A ``run_e3sm_diags.py`` example for running area mean time series alone:
    ...
    .. code:: python
        ...
        prefix = 'https://compy-dtn.pnl.gov/<username>/doc_examples/'
