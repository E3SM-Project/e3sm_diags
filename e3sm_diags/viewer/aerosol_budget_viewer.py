...
from .lat_lon_viewer import _create_lat_lon_table_index, _cvs_to_html
...
def create_viewer(root_dir, parameters):
    ...
    for season in seasons:
        csv_path = os.path.join(csv_table_dir, f"{parameters[0].test_name}-{season}-budget-table.csv")
        shutil.copy(csv_path, table_dir)

        csv_path = os.path.join(table_dir, f"{parameters[0].test_name}-{season}-budget-table.csv")
