...
TimeSelection = Union[ClimoFreq, TimeSlice]
...
class LatLonNativeParameter(CoreParameter):
    ...
    def check_values(self):
        ...
        """
        # Check that either seasons OR time_slices are specified, but not both
        ...
        # For now, we'll make grid file check a soft check
    ...
    def _validate_time_slice_format(self, time_slice: str):
        ...
        # Pattern for slice notation: optional_int:optional_int:optional_int
        # Also allow single integer
        pattern = r"^(-?\d+|)(?::(-?\d+|)(?::(-?\d+|))?)?$"
    ...
    def _set_time_slice_attrs(self, test_ds, ref_ds, time_slice: str):
        ...
        # For time slices, we manually set the name_yrs attributes instead of calling parent method
        # to avoid issues with the dataset's get_name_yrs_attr expecting a valid season

        # Set test_name_yrs - use test dataset years if available, otherwise use time slice info
        try:
    ...
    def _set_name_yrs_attrs(self, test_ds, ref_ds, season: Union[TimeSelection, None]):
        ...
        ref_ds : Dataset
            The reference dataset object.
        season : Union[ClimoFreq, str, None]
        ...
        if season is None or season in CLIMO_FREQS:
            ...
            super()._set_name_yrs_attrs(test_ds, ref_ds, season)  # type: ignore
        else:
