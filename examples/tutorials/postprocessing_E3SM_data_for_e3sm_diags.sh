...
#Note 1: For processing E3SM v2 output add '-P eam' or '-m eam' when calling ncclimo, also change 'cam' to 'eam' in this script
#Note 2: To specify start and end years, change 'start' and 'end', also change the years in {} when doing pip to stdin (i.e., {1980..2014}) for filename pattern match.
...
mkdir -p ${dir_out_climo}/rgr
mkdir -p ${dir_out_climo}/native
...
mkdir -p ${dir_out_diurnal_climo}/rgr
mkdir -p ${dir_out_diurnal_climo}/native
drc_rgr=${dir_out_diurnal_climo}/rgr
drc_out=${dir_out_diurnal_climo}/native
echo ${drc_in}

cd ${drc_in};ls ${caseid}.cam.h4.*{1980..2014}*.nc | ncclimo --clm_md=hfc --caseid=${caseid}.cam.h4 -v PRECT --ypf=1 --yr_srt=1980 --yr_end=2014 --drc_out=${drc_out} -O $drc_rgr --map=${map_file}
##
echo "Generating per-variable monthly time-series."
mkdir -p ${dir_out_ts}/rgr
mkdir -p ${dir_out_ts}/native
drc_rgr=${dir_out_ts}/rgr
drc_out=${dir_out_ts}/native

echo "Variables for Streamflow"
cd ${drc_in_river};ls ${caseid}*mosart.h0.*{1980..2014}*.nc | ncclimo  --caseid=${caseid} --var_xtr=areatotal2 -v RIVER_DISCHARGE_OVER_LAND_LIQ --yr_srt=$start --yr_end=$end --drc_out=${drc_rgr} 
#
echo "Variables for supporting diags using monthly time series as input(ENSO, QBO, etc.)"
cd ${drc_in};ls ${caseid}.cam.h0.*{1980..2014}*.nc | ncclimo  --caseid=${caseid} --var=U,CLDHGH,CLDLOW,CLDMED,CLDTOT,FLNS,FLUT,FSNS,FSNT,FSNTOA,LANDFRAC,LHFLX,LWCF,OCNFRAC,PRECC,PRECL,PSL,QFLX,SHFLX,SWCF,T,TAUX,TAUY,TREFHT,TS --yr_srt=$start --yr_end=$end --drc_out=${drc_out} -O $drc_rgr --map=${map_file}
done
